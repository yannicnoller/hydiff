...
public class BytecodeUtils {
	...
	/*
	 * Helper method to determine if the method just executed is one of the ones
	 * specified in the symbolic.method property Matched based on method name (short
	 * version) & number of params (if any) For methods without arguments, all
	 * methods with this name will be treated as symbolic methods since we cannot
		...
		if (methods != null) {
			...
			while (it.hasNext()) {
				...
				if (configMethodName.equalsIgnoreCase(shortName)) {
					// System.out.println("config method name "+configMethodName + " "+argNum + "
					// "+numberOfArgs);
	...
	/*
	 * Uses the class name to determine if a method is symbolic; use to declare all
	 * methods in the given class as symbolic
	 */
	...
	/**
	 * A container for BytecodeUtils.execute to return either the next instruction
	 * to execute or to tell the calling method to call super.execute();
	 *
	 *
	 */
	public static class InstructionOrSuper {
		...
		/**
		 * Ignore the value of 'inst' and just call the instruction's super.execute()
		 * method.
		 */
	...
	public static InstructionOrSuper execute(JVMInvokeInstruction invInst, ThreadInfo th) {
		...
		/*
		 * Here we test if the the method should be executed symbolically. We perform
		 * two checks: 1. Does the invoked method correspond to a method listed in the
		 * symbolic.method property and does the number of parameters match? 2. Is the
		 * method contained in a class that is to be executed symbolically? If the
		 * method is symbolic, initialize the parameter attributes and the fields if
		 * they are specified as symbolic based on annotations
		 *
		 */
		...
		// Start string handling: TODO corina it needs reviewing as it does not seem to
		// be correct
		...
		if (found) {
				...
				if (symClass || args.get(j).equalsIgnoreCase("SYM")) {
					...
					if (argTypes[j].equalsIgnoreCase("int")) {
						...
						if (symarray) {
							Object[] argValues = invInst.getArgumentValues(th);
							ElementInfo eiArray = (ElementInfo) argValues[j];
							// If the type name contains [] but wasn't catched previously, it is an object
							// array
							ArrayExpression sym_v = new ArrayExpression(th.getElementInfo(sf.peek()).toString(),
						...
						// throw new RuntimeException("## Error: parameter type not yet handled: " +
						// argTypes[j]);
					}
			...
			// this is pretty inefficient especially when preconditions are not used --
			// fixed somehow -- TODO: testing
