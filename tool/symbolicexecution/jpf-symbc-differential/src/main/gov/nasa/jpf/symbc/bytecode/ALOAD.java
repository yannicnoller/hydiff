...
public class ALOAD extends gov.nasa.jpf.jvm.bytecode.ALOAD {
    ...
    // private int numNewRefs = 0; // # of new reference objects to account for polymorphism -- work of Neha Rungta --
    // needs to be updated
	...
	public Instruction execute (ThreadInfo th) {
		...
		if(!th.isFirstStepInsn()) {
			...
			int increment = 2;
			if(typeClassInfo.isAbstract() || attr.toString().contains("this")) {
				 abstractClass = true;
		...
		else if (currentChoice == numSymRefs && !attr.toString().contains("this")){ //null object
			pcHeap._addDet(Comparator.EQ, (SymbolicInteger) attr, new IntegerConstant(-1));
			daIndex = MJIEnv.NULL;
		}
		else if ((currentChoice == (numSymRefs + 1) && !abstractClass) | (currentChoice == numSymRefs && attr.toString().contains("this"))) {
