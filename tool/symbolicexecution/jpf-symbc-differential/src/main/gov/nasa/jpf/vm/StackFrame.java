...
import gov.nasa.jpf.symbc.bytecode.shadow.*;
import gov.nasa.jpf.symbc.numeric.*;

/**
 * Describes callerSlots stack frame.
 *
 * Java methods always have bounded local and operand stack sizes, computed at compile time, stored in the classfile,
 * and checked at runtime by the bytecode verifier. Consequently, we combine locals and operands in one data structure
 * with the following layout
 *
 * slot[0] : 'this' .. .. local vars slot[stackBase-1] : last local var slot[stackBase] : first operand slot .. ^ .. |
 * operand stack range .. v slot[top] : highest used operand slot
 *
 *
 * The original implementation of this class belongs to the jpf-core project. It is being extended to use for the
 * jpf-shadow extension.
 */

public abstract class StackFrame implements Cloneable {

    /* jpf-shadow start modification */
    public void storeLongOperand(int index, ThreadInfo ti, Instruction insn) {
        /*
         * stores value from the top slot in the index slot long operands occupy two consecutive slots top value is
         * stored in slots[top-1] and slots[top] and will be stored into slots[index],slots[index+1]
         * 
         * the attributes (symbolic expressions/diffexpressions) are stored in the lower word
         */
        // long operands occupy two consecutive slots
        int t = top - 1; // lower index of the topmost value
        int i = index;

        Expression oldSymbolic_result = null;
        Expression oldConcrete_result = null;

        Expression newSymbolic_result = null;
        Expression newConcrete_result = null;

        if (attrs != null) {
            switch (ti.getExecutionMode()) {
            case BOTH: // store both the new and the old symbolic expressions
                attrs[index] = attrs[t]; // attributes are stored in the lower word
                attrs[index + 1] = null;

                // remove attributes from the top slots
                attrs[t] = null;
                attrs[t + 1] = null;
            ...
            case OLD: // we only want to store the old (shadow) symbolic expression

                // get the shadow symbolic expression from the top slot
                if (attrs[t] != null) {
                    if (attrs[t] instanceof DiffExpression) {
                        oldSymbolic_result = ((DiffExpression) attrs[t]).getOldSymbolicExpr();
                        oldConcrete_result = ((DiffExpression) attrs[t]).getNewConcreteExpr();
                    } else {
                        oldSymbolic_result = (Expression) attrs[t];
                        if (insn instanceof LSTORE) {
                            long concreteValue = this.getLongLocalVariable(t);
                            oldConcrete_result = new IntegerConstant((int) concreteValue);
                        } else {
                            assert (insn instanceof DSTORE);
                            double concreteValue = this.getDoubleLocalVariable(t);
                            oldConcrete_result = new RealConstant(concreteValue);
                        }
                    }
                } else {// top value is concrete
                    if (insn instanceof LSTORE) {
                        long concreteValue = this.getLongLocalVariable(t);
                        oldSymbolic_result = new IntegerConstant((int) concreteValue);
                        oldConcrete_result = new IntegerConstant((int) concreteValue);
                    } else {
                        assert (insn instanceof DSTORE);
                        double concreteValue = this.getDoubleLocalVariable(t);
                        oldSymbolic_result = new RealConstant(concreteValue);
                        oldConcrete_result = new RealConstant(concreteValue);
                    }
                }

                // save the sym expression from the index slot
                if (attrs[index] != null) {
                    if (attrs[index] instanceof DiffExpression) {
                        newSymbolic_result = ((DiffExpression) attrs[index]).getNewSymbolicExpr();
                        newConcrete_result = ((DiffExpression) attrs[index]).getNewConcreteExpr();
                    } else {
                        newSymbolic_result = (Expression) attrs[index];
                        if (insn instanceof LSTORE) {
                            long concreteValue = this.getLongLocalVariable(index);
                            newConcrete_result = new IntegerConstant((int) concreteValue);
                        } else {
                            assert (insn instanceof DSTORE);
                            double concreteValue = this.getDoubleLocalVariable(index);
                            newConcrete_result = new RealConstant(concreteValue);
                        }
                    }
                } else {// index value is concrete
                    if (insn instanceof LSTORE) {
                        long concreteValue = this.getLongLocalVariable(index);
                        newSymbolic_result = new IntegerConstant(concreteValue);
                        newConcrete_result = new IntegerConstant(concreteValue);
                    } else {
                        assert (insn instanceof DSTORE);
                        double concreteValue = this.getDoubleLocalVariable(index);
                        newSymbolic_result = new RealConstant(concreteValue);
                        newConcrete_result = new RealConstant(concreteValue);
                    }
                }
                break;

            case NEW: // we only want to store the new symbolic expression

                // get the new symbolic expression from the top slot
                if (attrs[t] != null) {
                    if (attrs[t] instanceof DiffExpression) {
                        newSymbolic_result = ((DiffExpression) attrs[t]).getNewSymbolicExpr();
                        newConcrete_result = ((DiffExpression) attrs[t]).getNewConcreteExpr();
                    } else {
                        newSymbolic_result = (Expression) attrs[t];
                        if (insn instanceof LSTORE) {
                            long concreteValue = this.getLongLocalVariable(t);
                            newConcrete_result = new IntegerConstant((int) concreteValue);
                        } else {
                ...
                } else {// concrete value, create a constant expression for the diffexpression
                    if (insn instanceof LSTORE) {
                        long concreteValue = this.getLongLocalVariable(t);
                        newSymbolic_result = new IntegerConstant((int) concreteValue);
                        newConcrete_result = new IntegerConstant((int) concreteValue);
                    } else {
                        assert (insn instanceof DSTORE);
                        double concreteValue = this.getDoubleLocalVariable(t);
                ...
                // save the old shadow expression since we dont want to modify it
                if (attrs[index] != null) {
                    if (attrs[index] instanceof DiffExpression) {
                        oldSymbolic_result = ((DiffExpression) attrs[index]).getOldSymbolicExpr();
                        oldConcrete_result = ((DiffExpression) attrs[index]).getNewConcreteExpr();
                    } else {
                        oldSymbolic_result = (Expression) attrs[index];
                        if (insn instanceof LSTORE) {
                            long concreteValue = this.getLongLocalVariable(index);
                            oldConcrete_result = new IntegerConstant((int) concreteValue);
                        } else {
            ...
            DiffExpression result = new DiffExpression(oldSymbolic_result, newSymbolic_result, oldConcrete_result,
                    newConcrete_result);
            attrs[index] = result;
            attrs[index + 1] = null;
            ...
            // TODO: we might want to create a diffexpression here if the executionmode is
            // old or new
        }

        // now we can modify the concrete values
        slots[i] = slots[t];
        isRef.clear(i);
    ...
    public void storeOperand(int index, ThreadInfo ti, Instruction insn) {
        ...
        if (attrs != null) {
            /*
             * In the cases OLD and NEW we only want to change the shadow expression or the symbolic expression,
             * respectively. Therefore, the object we store as the attribute value will always be a DiffExpression.
             * However, the case BOTH will simply store the attribute value of the top slot (not necessarily a
             * DiffExpr).
             */
            switch (ti.getExecutionMode()) {
            case BOTH:
                ...
                return;
            // break;
            case OLD: // we only want to modify the symbolic expression
                // get the new symbolic expression from the top slot
                if (attrs[top] != null) {
                    if (attrs[top] instanceof DiffExpression) {
                        oldSymbolic_result = ((DiffExpression) attrs[top]).getOldSymbolicExpr();
                        oldConcrete_result = ((DiffExpression) attrs[top]).getOldConcreteExpr();
                    } else {
                        oldSymbolic_result = (Expression) attrs[top];
                        oldConcrete_result = new IntegerConstant(slots[top]);
                    }
                } else {
                    if (insn instanceof ISTORE) {
                        oldSymbolic_result = new IntegerConstant(slots[top]);
                        oldConcrete_result = new IntegerConstant(slots[top]);
                    } else {
                        assert (insn instanceof FSTORE);
                        float concreteValue = this.getFloatLocalVariable(top);
                        oldSymbolic_result = new RealConstant(concreteValue);
                        oldConcrete_result = new RealConstant(concreteValue);
                    }
                }
                // save the current shadow expression from the index slot since we dont want to
                // modify it
                if (attrs[index] != null) {
                    if (attrs[index] instanceof DiffExpression) {
                        newSymbolic_result = ((DiffExpression) attrs[index]).getNewSymbolicExpr();
                        ...
                        newSymbolic_result = (Expression) attrs[index];
                        newConcrete_result = new IntegerConstant(slots[index]);
                    }
                } else {
                    if (insn instanceof ISTORE) {
                        newSymbolic_result = new IntegerConstant(slots[index]);
                        newConcrete_result = new IntegerConstant(slots[index]);
                    } else {
                        assert (insn instanceof FSTORE);
                        float concreteValue = this.getFloatLocalVariable(index);
                        newSymbolic_result = new RealConstant(concreteValue);
                        newConcrete_result = new RealConstant(concreteValue);
                    }
                }
                break;
            case NEW:
                // we only want to modify the shadow expression, therefore we only need the new
                // shadow expression
                if (attrs[top] != null) {
                    if (attrs[top] instanceof DiffExpression) {
                        newSymbolic_result = ((DiffExpression) attrs[top]).getNewSymbolicExpr();
                        newConcrete_result = ((DiffExpression) attrs[top]).getNewConcreteExpr();
                    } else {
                        newSymbolic_result = (Expression) attrs[top];
                        newConcrete_result = new IntegerConstant(slots[top]);
                    }
                } else {
                    if (insn instanceof ISTORE) {
                        newSymbolic_result = new IntegerConstant(slots[top]);
                        newConcrete_result = new IntegerConstant(slots[top]);
                    } else {
                        assert (insn instanceof FSTORE);
                        float concreteValue = this.getFloatLocalVariable(top);
                ...
                // save the current symbolic expression since it must not be modified
                if (attrs[index] != null) {
                    if (attrs[index] instanceof DiffExpression) {
                        oldSymbolic_result = ((DiffExpression) attrs[index]).getOldSymbolicExpr();
                        oldConcrete_result = ((DiffExpression) attrs[index]).getOldConcreteExpr();
                    } else {
                        oldSymbolic_result = (Expression) attrs[index];
                        oldConcrete_result = new IntegerConstant(slots[index]);
                    }
                } else {
                    if (insn instanceof ISTORE) {
                        oldSymbolic_result = new IntegerConstant(slots[index]);
                        oldConcrete_result = new IntegerConstant(slots[index]);
                    } else {
                        assert (insn instanceof FSTORE);
                        float concreteValue = this.getFloatLocalVariable(index);
        ...
        // now we can also modify the concrete values
        slots[index] = slots[top];
        isRef.set(index, isRef.get(top));
        top--;
    ...
    /**
     * this StackFrame is not allowed to be modified anymore because it has been state stored. Set during state storage
     * and checked upon each modification, causing exceptions on attempts to modify callerSlots frozen instance. The
     * flag is reset in clones
     */
    public static final int ATTR_IS_FROZEN = 0x100;
    static final int ATTR_IS_REFLECTION = 0x1000;
    /**
     * the previous StackFrame (usually the caller, null if first). To be set when the frame is pushed on the ThreadInfo
     * callstack
     */
    protected StackFrame prev;

    /**
     * state management related attributes similar to ElementInfo. The lower 16 bits are stored/restored, the upper 16
     * bits are for transient use
     */
    ...
    /*
     * This array can be used to store attributes (e.g. variable names) for operands. We don't do anything with this
     * except of preserving it (across dups etc.), so it's pretty much up to the VM listeners/peers what's stored
     *
     * NOTE: attribute values are not restored upon backtracking per default, but attribute references are. If you need
     * restoration of values, use copy-on-write in your clients
     *
    ...
    /**
     * creates callerSlots dummy Stackframe for testing of operand/local operations NOTE - TESTING ONLY! this does not
     * have a MethodInfo
     */
    ...
    /**
     * return the object reference for an instance method to be called (we are still in the caller's frame). This only
     * makes sense after all params have been pushed, before the INVOKEx insn is executed
     */
    ...
    /**
     * to be set (by ThreadInfo) when the frame is pushed. Can also be used for non-local gotos, but be warned - that's
     * tricky
     */
    ...
    /**
     * does any of the 'nTopSlots' hold callerSlots reference value of 'objRef' 'nTopSlots' is usually obtained from
     * MethodInfo.getNumberOfCallerStackSlots()
     */
    ...
    /**
     * this returns all of them - use either if you know there will be only one attribute at callerSlots time, or
     * check/process result with ObjectList
     */
    ...
    /**
     * this replaces all of them - use only if you know there are no SystemAttributes in the list (which would cause an
     * exception)
     */
    ...
    /**
     * this only returns the first attr of this type, there can be more if you don't use client private types or the
    ...
    /**
     * this replaces all of them - use only if you know - there will be only one attribute at callerSlots time - you
     * obtained the value you set by callerSlots previous getXAttr() - you constructed callerSlots multi value list with
    ...
    // <2do> this is machine dependent since it uses the operand stack. Only here
    // because there
    // is no suitable place to factor this out between xStackFrame,
    // xNativeStackFrame and xDirectCallStackFrame
     ...
     * 
     * references are returned as ElementInfos or null primitive values are returned as box objects (e.g. int ->
     * Integer)
     */
    ...
    /**
     * get the arguments of the executed call Note - this throws an exception if the StackFrame pc is not an
     * InvokeInstruction
     */
    ...
    /**
     * return an array of all argument attrs, which in turn can be lists. If you have to retrieve values, use the
     * ObjectList APIs
     * 
     * this is here (and not in ThreadInfo) because we might call it on callerSlots cached/cloned StackFrame (caller
     * stack might be already modified, e.g. for callerSlots native method). to be used from listeners.
     */
    ...
    /**
     * check if there is any argument attr of the provided type on the operand stack this is far more efficient than
     * retrieving attribute values (we don't care for argument types)
     */
    ...
    /**
     * if this is an instance method, return the reference of the corresponding object (note this only has to be in slot
     * 0 upon entry)
     */
    public int getThis() {
        return thisRef;
    ...
    /**
     * to be used to check if a StackFrame got cloned due to its execution changing attributes and/or slots, but
     * otherwise represents the same execution
     */
    public boolean originatesFrom(StackFrame other) {
        if (other == this) {
            return true;
        } else {
            return ((mi == other.mi) && (prev == other.prev) && (top == other.top) && (getClass() == other.getClass()));
        }
    }
    ...
    public int mixinExecutionStateHash(int h) {
        ...
        if (pc != null) {
            ...
            // we don't need the bytecode since there can only be one insn with this index
            // in this method
        }
    ...
    /**
     * mark all objects reachable from local or operand stack positions containing references. Done during phase1
     * marking of threads (the stack is one of the Thread gc roots)
     */
    public void markThreadRoots(Heap heap, int tid) {

        /**
         * for (int i = isRef.nextSetBit(0); i>=0 && i<=top; i = isRef.nextSetBit(i + 1)) { int objref = slots[i]; if
         * (objref != MJIEnv.NULL) { heap.markThreadRoot(objref, tid); } }
         **/
        for (int i = 0; i <= top; i++) {
            if (isRef.get(i)) {
                int objref = slots[i];
    ...
    // --- abstract argument & return passing that can have VM dependend
    // implementation
