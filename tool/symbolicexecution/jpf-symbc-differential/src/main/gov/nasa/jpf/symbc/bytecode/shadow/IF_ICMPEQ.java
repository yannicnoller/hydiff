            ...
            if (!ti.isFirstStepInsn()) {
                ...
                if (SymbolicInstructionFactory.collect_constraints) {
                    ...
                    if (pc.isDiffPC()) {
                        ...
                        pc._addDet(newConcrete_result ? Comparator.EQ : Comparator.NE, newSym_v1, newSym_v2);
                        ...
                        if (!oldConcrete_result) {
                            ...
                            if (ti.getExecutionMode() == Execute.NEW || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.NE, newSym_v1, newSym_v2);
                            }
                            if (ti.getExecutionMode() == Execute.OLD || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.NE, oldSym_v1, oldSym_v2);
                            }
                            curCg.setCurrentPC(pc);
                            ...
                            curCg.select(1);
                            if (ti.getExecutionMode() == Execute.NEW || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.EQ, newSym_v1, newSym_v2);
                            }
                            if (ti.getExecutionMode() == Execute.OLD || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.EQ, oldSym_v1, oldSym_v2);
                            }
                            curCg.setCurrentPC(pc);
                        ...
                        if (ti.getExecutionMode() == Execute.OLD) {
                            pc._addDet(oldConcrete_result ? Comparator.EQ : Comparator.NE, oldSym_v1, oldSym_v2);
                    ...
                    case 2: // Diff true path in src (new true, old false)
                        ...
                        return this.getNext(ti); // continue new version
                    ...
                    case 3: // Diff false path in src (new false, old true)
                        ...
                        return this.getTarget(); // continue new version

                    default:
                        throw new RuntimeException("Unhandled choice in IF_ICMPEQ: " + choice);
                    }
                }
            }
        }
