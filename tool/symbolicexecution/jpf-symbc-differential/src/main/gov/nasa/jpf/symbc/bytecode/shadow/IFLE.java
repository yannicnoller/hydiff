...
//A PARTICULAR PURPOSE, OR FREEDOM FROM INFRINLEMENT, ANY WARRANTY THAT
            ...
            if (!ti.isFirstStepInsn()) {
                ...
                if (SymbolicInstructionFactory.collect_constraints) {
                    ...
                    if (pc.isDiffPC()) {
                        ...
                        pc._addDet(newConcrete_result ? Comparator.LE : Comparator.GT, newSym_v, 0);
                        ...
                        if (!oldConcrete_result) {
                            ...
                            if (ti.getExecutionMode() == Execute.NEW || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.GT, newSym_v, 0);
                            }
                            if (ti.getExecutionMode() == Execute.OLD || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.GT, oldSym_v, 0);
                            }
                            curCg.setCurrentPC(pc);
                            ...
                            curCg.select(1);
                            if (ti.getExecutionMode() == Execute.NEW || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.LE, newSym_v, 0);
                            }
                            if (ti.getExecutionMode() == Execute.OLD || ti.getExecutionMode() == Execute.BOTH) {
                                pc._addDet(Comparator.LE, oldSym_v, 0);
                            }
                            curCg.setCurrentPC(pc);
                        ...
                        if (ti.getExecutionMode() == Execute.OLD) {
                            pc._addDet(oldConcrete_result ? Comparator.LE : Comparator.GT, oldSym_v, 0);
                    ...
                    case 3: // Diff false path in src (new false, old true)
                        ...
                        return this.getTarget(); // continue with new version
                    default:
                        throw new RuntimeException("Unhandled choice in IFLE: " + choice);
                    }
                }
            }
        }
