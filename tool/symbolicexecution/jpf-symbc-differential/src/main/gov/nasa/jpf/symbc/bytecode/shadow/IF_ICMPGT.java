			...
			if (!ti.isFirstStepInsn()) {
				...
				if (SymbolicInstructionFactory.collect_constraints) {
					...
					if (pc.isDiffPC()) {
						...
						pc._addDet(newConcrete_result ? Comparator.GT : Comparator.LE, newSym_v1, newSym_v2);
						...
						if (!oldConcrete_result) {
							...
							if (ti.getExecutionMode() == Execute.NEW || ti.getExecutionMode() == Execute.BOTH) {
								pc._addDet(Comparator.LE, newSym_v1, newSym_v2);
							}
							if (ti.getExecutionMode() == Execute.OLD || ti.getExecutionMode() == Execute.BOTH) {
								pc._addDet(Comparator.LE, oldSym_v1, oldSym_v2);
							}
							curCg.setCurrentPC(pc);
							...
							curCg.select(1);
							if (ti.getExecutionMode() == Execute.NEW || ti.getExecutionMode() == Execute.BOTH) {
								pc._addDet(Comparator.GT, newSym_v1, newSym_v2);
							}
							if (ti.getExecutionMode() == Execute.OLD || ti.getExecutionMode() == Execute.BOTH) {
								pc._addDet(Comparator.GT, oldSym_v1, oldSym_v2);
							}
							curCg.setCurrentPC(pc);
						...
						if (ti.getExecutionMode() == Execute.OLD) {
							pc._addDet(oldConcrete_result ? Comparator.GT : Comparator.LE, oldSym_v1, oldSym_v2);
					...
					default:
						throw new RuntimeException("Unhandled choice in IF_ICMPGT: " + choice);
					}
				}
			}
		}
