...
public class ThreadInfo extends InfoObject
  ...
  //jpf-shadow: execution mode of the thread, used by if-insns in change(boolean,boolean) stmts in order to properly determine constraints
  public enum Execute {
    OLD, //means only the old version is being executed, e.g. for removed statements of a patch
    BOTH, //both versions are being executed simultaneously
    NEW //only the new version is being executed, e.g. if new statements were added 
  }
  
  private Execute executionMode = Execute.BOTH;
  
  public Execute getExecutionMode(){
	  return this.executionMode;
  }
  
  public void setExecutionMode(Execute em){
