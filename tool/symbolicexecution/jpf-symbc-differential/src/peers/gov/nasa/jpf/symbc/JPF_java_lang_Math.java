...
public class JPF_java_lang_Math extends NativePeer {
    ...
    public static double sqrt__D__D(MJIEnv env, int clsObjRef, double a) {
        ...
        if (attrs[0] instanceof DiffExpression) {
            /* YN: extension for DiffExpression */
            
            RealExpression oldSym = ShadowBytecodeUtils.getOldSymbolicExpr(attrs[0], a);
            RealExpression oldCon = ShadowBytecodeUtils.getOldConcreteExpr(attrs[0], a);
            RealExpression newSym = ShadowBytecodeUtils.getNewSymbolicExpr(attrs[0], a);
            RealExpression newCon = ShadowBytecodeUtils.getNewConcreteExpr(attrs[0], a);
            ...
            if (env.getThreadInfo().getExecutionMode() == Execute.OLD || env.getThreadInfo().getExecutionMode() == Execute.BOTH) {
                if (env.getThreadInfo().getExecutionMode() == Execute.OLD) {
                    newSym_result = new RealConstant(0);
                    newCon_result = new RealConstant(0);
                }
                oldSym_result = new MathRealExpression(MathFunction.SQRT, oldSym);
                oldCon_result = new RealConstant(Math.sqrt(oldCon.solution()));
            }

            if (env.getThreadInfo().getExecutionMode() == Execute.NEW || env.getThreadInfo().getExecutionMode() == Execute.BOTH) {
                if (env.getThreadInfo().getExecutionMode() == Execute.NEW) {
                    oldSym_result = new RealConstant(0);
                    oldCon_result = new RealConstant(0);
                }
                newSym_result = new MathRealExpression(MathFunction.SQRT, newSym);
                newCon_result = new RealConstant(Math.sqrt(newCon.solution()));
            }
    ...
    public static double toRadians__D__D(MJIEnv env, int clsObjRef, double a) {
        ...
        if (sym_arg == null) // concrete
            ...
            sym_arg._mul(new RealConstant(180.0 * Math.PI));
            RealExpression result = new MathRealExpression(MathFunction.COS, sym_arg);
            env.setReturnAttribute(result);
            // System.out.println("result "+result);
            return 0;
