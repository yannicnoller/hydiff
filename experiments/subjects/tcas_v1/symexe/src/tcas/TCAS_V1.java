...
public class TCAS_V1 {
	...
	private boolean Non_Crossing_Biased_Climb(){
	    ...
	    if (upward_preferred){
	        boolean tmp = change(Down_Separation >= ALIM(), Down_Separation > ALIM());
	    	result = !(Own_Below_Threat()) || ((Own_Below_Threat()) && (!(tmp))); // operator mutation, change: >= to >
	    }else{	
