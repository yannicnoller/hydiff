/*
 * revision: 67dffec4e7adc482215df440e716525ffce5c1c5
...
import test.org.apache.commons.math3.util.ArithmeticUtils;
import test.org.apache.commons.math3.util.FastMath;
import test.org.apache.commons.math3.util.MathArrays;

/**
 * Class holding "compiled" computation rules for derivative structures.
 * <p>
 * This class implements the computation rules described in Dan Kalman's paper
 * <a href="http://www.math.american.edu/People/kalman/pdffiles/mmgautodiff.pdf">Doubly Recursive Multivariate Automatic
 ...
 * derivation orders and {@link #getPartialDerivativeOrders(int) getPartialDerivativeOrders} returns an array filled
 * with 0 when called with 0 as the index).
 * </p>
 * <p>
 ...
 * 
 * <pre>
 * // compute the product of all elements double[] product = new double[size]; prod[0] = 1.0; for (int i = 0; i < n;
 * ++i) { double[] tmp = product.clone(); compiler.multiply(tmp, 0, array, i * size, product, 0); }
 *
 * // value double p = product[0];
 *
 * // first derivatives double dPdX = product[compiler.getPartialDerivativeIndex(1, 0, 0)]; double dPdY =
 * product[compiler.getPartialDerivativeIndex(0, 1, 0)]; double dPdZ = product[compiler.getPartialDerivativeIndex(0, 0,
 * 1)];
 *
 * // cross derivatives (assuming order was at least 2) double dPdXdX = product[compiler.getPartialDerivativeIndex(2, 0,
 * 0)]; double dPdXdY = product[compiler.getPartialDerivativeIndex(1, 1, 0)]; double dPdXdZ =
 * product[compiler.getPartialDerivativeIndex(1, 0, 1)]; double dPdYdY = product[compiler.getPartialDerivativeIndex(0,
 * 2, 0)]; double dPdYdZ = product[compiler.getPartialDerivativeIndex(0, 1, 1)]; double dPdZdZ =
 * product[compiler.getPartialDerivativeIndex(0, 0, 2)];
 * </p>
 * 
 ...
 * @since 3.1
 */
public class DSCompiler_v1 {

    /** Array of all compilers created so far. */
    private static AtomicReference<DSCompiler_v1[][]> compilers = new AtomicReference<DSCompiler_v1[][]>(null);
     ...
     */
    public static DSCompiler_v1 getCompiler(int parameters, int order) {

        // get the cached compilers
        final DSCompiler_v1[][] cache = compilers.get();
        ...
        final DSCompiler_v1[][] newCache = new DSCompiler_v1[maxParameters + 1][maxOrder + 1];
        ...
        for (int diag = 0; diag <= maxParameters + maxOrder; ++diag) {
            for (int o = FastMath.max(0, diag - maxParameters); o <= FastMath.min(maxOrder, diag); ++o) {
                ...
                if (newCache[p][o] == null) {
                    final DSCompiler_v1 valueCompiler = (p == 0) ? null : newCache[p - 1][o];
                    final DSCompiler_v1 derivativeCompiler = (o == 0) ? null : newCache[p][o - 1];
                    newCache[p][o] = new DSCompiler_v1(p, o, valueCompiler, derivativeCompiler);
                }
            }
        }
     ...
     */
    private DSCompiler_v1(final int parameters, final int order, final DSCompiler_v1 valueCompiler,
            final DSCompiler_v1 derivativeCompiler) {

        this.parameters = parameters;
        this.order = order;
        this.sizes = compileSizes(parameters, order, valueCompiler, derivativeCompiler);
        this.derivativesIndirection = compileDerivativesIndirection(parameters, order, valueCompiler,
                derivativeCompiler);
        this.lowerIndirection = compileLowerIndirection(parameters, order, valueCompiler, derivativeCompiler);
        this.multIndirection = compileMultiplicationIndirection(parameters, order, valueCompiler, derivativeCompiler,
                lowerIndirection);
        this.compIndirection = compileCompositionIndirection(parameters, order, valueCompiler, derivativeCompiler,
     ...
     * @return sizes array
     */
    private static int[][] compileSizes(final int parameters, final int order, final DSCompiler_v1 valueCompiler,
            final DSCompiler_v1 derivativeCompiler) {
    ...
    private static int[][] compileDerivativesIndirection(final int parameters, final int order,
            final DSCompiler_v1 valueCompiler, final DSCompiler_v1 derivativeCompiler) {
        ...
        for (int i = 0; i < dSize; ++i) {
            ...
            // copy the indices
            System.arraycopy(derivativeCompiler.derivativesIndirection[i], 0, derivativesIndirection[vSize + i], 0,
                    parameters);
    ...
    private static int[][][] compileMultiplicationIndirection(final int parameters, final int order,
            final DSCompiler_v1 valueCompiler, final DSCompiler_v1 derivativeCompiler, final int[] lowerIndirection) {
    ...
    private static int[][][] compileCompositionIndirection(final int parameters, final int order,
            final DSCompiler_v1 valueCompiler, final DSCompiler_v1 derivativeCompiler, final int[][] sizes,
            final int[][] derivativesIndirection, final int[] lowerIndirection) {
     ...
     * function. The index for this 0<sup>th</sup> order derivative is always 0. the indices of higher order derivatives
     * is between 1 and {@link #getSize() - 1)}.
     * </p>
     * <p>
     ...
     *            sizes array
     * @param orders
     *            derivation orders with respect to each parameter (the lenght of this array must match the number of
     *            parameters)
     * @return index of the partial derivative
     * @exception NumberIsTooLargeException
     *                if sum of derivation orders is larger than the instance limits
     */
    private static int getPartialDerivativeIndex(final int parameters, final int order, final int[][] sizes,
     ...
     *            derivatives indirection array for the source derivative structure
     * @param destP
     ...
     * @return index of the partial derivative with the <em>same</em> characteristics in destination derivative
     *         structure
     */
     ...
     */
    public void linearCombination(final double a1, final double[] c1, final int offset1, final double a2,
            final double[] c2, final int offset2, final double[] result, final int resultOffset) {
     ...
     */
    public void linearCombination(final double a1, final double[] c1, final int offset1, final double a2,
            final double[] c2, final int offset2, final double a3, final double[] c3, final int offset3,
            final double[] result, final int resultOffset) {
        for (int i = 0; i < getSize(); ++i) {
            result[resultOffset + i] = MathArrays.linearCombination(a1, c1[offset1 + i], a2, c2[offset2 + i], a3,
                    c3[offset3 + i]);
        }
    }
    ...
    public void linearCombination(final double a1, final double[] c1, final int offset1, final double a2,
            final double[] c2, final int offset2, final double a3, final double[] c3, final int offset3,
            final double a4, final double[] c4, final int offset4, final double[] result, final int resultOffset) {
        for (int i = 0; i < getSize(); ++i) {
            result[resultOffset + i] = MathArrays.linearCombination(a1, c1[offset1 + i], a2, c2[offset2 + i], a3,
     ...
     *            offset of the result in its array
     */
    public void add(final double[] lhs, final int lhsOffset, final double[] rhs, final int rhsOffset,
            final double[] result, final int resultOffset) {
        for (int i = 0; i < getSize(); ++i) {
     ...
     *            offset of the result in its array
     */
    public void subtract(final double[] lhs, final int lhsOffset, final double[] rhs, final int rhsOffset,
            final double[] result, final int resultOffset) {
        for (int i = 0; i < getSize(); ++i) {
            result[resultOffset + i] = lhs[lhsOffset + i] - rhs[rhsOffset + i];
        }
     ...
     *            offset of the right hand side in its array
     * @param result
     *            array where result must be stored (for multiplication the result array <em>cannot</em> be one of the
     *            input arrays)
     * @param resultOffset
     *            offset of the result in its array
     */
    public void multiply(final double[] lhs, final int lhsOffset, final double[] rhs, final int rhsOffset,
            final double[] result, final int resultOffset) {
     ...
     *            offset of the right hand side in its array
     * @param result
     *            array where result must be stored (for division the result array <em>cannot</em> be one of the input
     *            arrays)
     * @param resultOffset
     *            offset of the result in its array
     */
    public void divide(final double[] lhs, final int lhsOffset, final double[] rhs, final int rhsOffset,
            final double[] result, final int resultOffset) {
     ...
     *            offset of the result in its array
     */
    public void remainder(final double[] lhs, final int lhsOffset, final double[] rhs, final int rhsOffset,
            final double[] result, final int resultOffset) {
     ...
     *            power to apply
     * @param result
     *            array where result must be stored (for power the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
    public void pow(final double[] operand, final int operandOffset, final double p, final double[] result,
     ...
     *            offset of the result in its array
     */
    public void pow(final double[] operand, final int operandOffset, final int n, final double[] result,
            final int resultOffset) {
     ...
     *            offset of the result in its array
     */
    public void pow(final double[] x, final int xOffset, final double[] y, final int yOffset, final double[] result,
            final int resultOffset) {
     ...
     *            order of the root
     * @param result
     *            array where result must be stored (for n<sup>th</sup> root the result array <em>cannot</em> be the
     *            input array)
     * @param resultOffset
     *            offset of the result in its array
     */
    public void rootN(final double[] operand, final int operandOffset, final int n, final double[] result,
            final int resultOffset) {
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for exponential the result array <em>cannot</em> be the input
     *            array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for logarithm the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for cosine the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for sine the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for tangent the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
        ...
        if (order > 0) {
            ...
            for (int n = 2; n <= order; ++n) {
                ...
                for (int k = 0; k < n; ++k) {
                    ...
                    outerSum += sign * innerSum * ArithmeticUtils.binomialCoefficient(n - 1, k) * twoNm2K * secant2Kp2
                            / (k + 1);
                    sign = -sign;
                    secant2Kp2 *= secant2;
                }
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for arc cosine the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for arc sine the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for arc tangent the result array <em>cannot</em> be the input
     *            array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     * @param result
     *            array where result must be stored (for two arguments arc tangent the result array <em>cannot</em> be
     *            the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
    public void atan2(final double[] y, final int yOffset, final double[] x, final int xOffset, final double[] result,
        ...
        if (x[xOffset] >= 0) {
            ...
            divide(y, yOffset, tmp2, 0, tmp1, 0); // y /(r - x)
            atan(tmp1, 0, tmp2, 0); // atan(y / (r - x))
            result[resultOffset] = ((tmp2[0] <= 0) ? -FastMath.PI : FastMath.PI) - 2 * tmp2[0]; // +/-pi - 2 * atan(y /
                                                                                                // (r - x))
        ...
        // result[resultOffset] = FastMath.atan2(y[yOffset], x[xOffset]); // insert fix
     ...
     *            offset of the operand in its array
     * @param f
     *            array of value and derivatives of the function at the current point (i.e. at
     *            {@code operand[operandOffset]}).
     * @param result
     *            array where result must be stored (for composition the result array <em>cannot</em> be the input
     *            array)
     * @param resultOffset
     *            offset of the result in its array
     */
    public void compose(final double[] operand, final int operandOffset, final double[] f, final double[] result,
            final int resultOffset) {
     ...
     */
    public void checkCompatibility(final DSCompiler_v1 compiler) throws DimensionMismatchException {
