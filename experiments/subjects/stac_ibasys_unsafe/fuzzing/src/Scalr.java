...
public class Scalr {
    ...
    public static final ConvolveOp OP_ANTIALIAS = new ConvolveOp(
            new Kernel(3, 3, new float[] { 0.0F, 0.08F, 0.0F, 0.08F, 0.68F, 0.08F, 0.0F, 0.08F, 0.0F }), 1,
            (RenderingHints) null);
    ...
    // public static final ColorConvertOp OP_GRAYSCALE = new ColorConvertOp(ColorSpace.getInstance(1003),
    // (RenderingHints)null); // TODO YN: SPF cannot handle this.
                ...
                if (op != null) {
                    if (DEBUG) {
                        log(1, "Applying BufferedImageOp [class=%s, toString=%s]...",
                                new Object[] { op.getClass(), op.toString() });
                    }
                    ...
                    if (DEBUG) {
                        log(1, "Applied BufferedImageOp in %d ms, result [width=%d, height=%d]",
                                new Object[] { Long.valueOf(System.currentTimeMillis() - subT),
                                        Integer.valueOf(result.getWidth()), Integer.valueOf(result.getHeight()) });
                    }
                }
            }

            if (DEBUG) {
                log(0, "All %d BufferedImageOps applied in %d ms",
                        new Object[] { Integer.valueOf(ops.length), Long.valueOf(System.currentTimeMillis() - t) });
            }
            ...
            if (x + width > srcWidth) {
                ...
                if (DEBUG) {
                    log(0, "Cropping Image [width=%d, height=%d] to [x=%d, y=%d, width=%d, height=%d]...",
                            new Object[] { Integer.valueOf(srcWidth), Integer.valueOf(srcHeight), Integer.valueOf(x),
                                    Integer.valueOf(y), Integer.valueOf(width), Integer.valueOf(height) });
                }

                BufferedImage result = createOptimalImage(src, width, height);
                // -> start of modification by YN
                // TODO YN: Graphics is not supported in SPF
                // Graphics g = result.getGraphics();
                // g.drawImage(src, 0, 0, width, height, x, y, x + width, y + height, (ImageObserver) null);
                // g.dispose();
                for (int i = 0; i < width; i++) {
                    for (int j = 0; j < height; j++) {
                        int pixel = result.getRGB(x + i, y + j);
        ...
        if (src == null) {
            ...
            if (DEBUG) {
                log(0, "Padding Image from [originalWidth=%d, originalHeight=%d, padding=%d] to [newWidth=%d, newHeight=%d]...",
                        new Object[] { Integer.valueOf(srcWidth), Integer.valueOf(srcHeight), Integer.valueOf(padding),
                                Integer.valueOf(newWidth), Integer.valueOf(newHeight) });
            }
    ...
    public static BufferedImage resize(BufferedImage src, Scalr.Method scalingMethod, int targetSize,
    ...
    public static BufferedImage resize(BufferedImage src, Scalr.Mode resizeMode, int targetSize, BufferedImageOp... ops)
            throws IllegalArgumentException, ImagingOpException {
        return resize(src, Scalr.Method.AUTOMATIC, resizeMode, targetSize, targetSize, ops);
    }

    public static BufferedImage resize(BufferedImage src, Scalr.Method scalingMethod, Scalr.Mode resizeMode,
            int targetSize, BufferedImageOp... ops) throws IllegalArgumentException, ImagingOpException {
    ...
    public static BufferedImage resize(BufferedImage src, Scalr.Method scalingMethod, int targetWidth, int targetHeight,
            BufferedImageOp... ops) {
        return resize(src, scalingMethod, Scalr.Mode.AUTOMATIC, targetWidth, targetHeight, ops);
    }

    public static BufferedImage resize(BufferedImage src, Scalr.Mode resizeMode, int targetWidth, int targetHeight,
            BufferedImageOp... ops) throws IllegalArgumentException, ImagingOpException {
        ...
        if (src == null) {
            ...
            if (DEBUG) {
                log(0, "Resizing Image [size=%dx%d, resizeMode=%s, orientation=%s, ratio(H/W)=%f] to [targetSize=%dx%d]",
                        new Object[] { Integer.valueOf(currentWidth), Integer.valueOf(currentHeight), resizeMode,
                                ratio <= 1.0F ? "Landscape/Square" : "Portrait", Float.valueOf(ratio),
            ...
            if (resizeMode == Scalr.Mode.FIT_EXACT) {
                ...
                if (DEBUG) {
                    log(1, "Auto-Corrected width and height based on scalingRatio %d.",
                            new Object[] { Float.valueOf(actualScaling) });
                }
            } else {
                ...
                if ((ratio > 1.0F || resizeMode != Scalr.Mode.AUTOMATIC) && resizeMode != Scalr.Mode.FIT_TO_WIDTH) {
                    ...
                    if (DEBUG && originalTargetWidth1 != targetWidth) {
                        log(1, "Auto-Corrected targetWidth [from=%d to=%d] to honor image proportions.",
                                new Object[] { Integer.valueOf(originalTargetWidth1), Integer.valueOf(targetWidth) });
                    }
                } else {
                    ...
                    if (DEBUG && originalTargetWidth1 != targetHeight) {
                        log(1, "Auto-Corrected targetHeight [from=%d to=%d] to honor image proportions.",
                                new Object[] { Integer.valueOf(originalTargetWidth1), Integer.valueOf(targetHeight) });
                    }
                }
            }
    ...
    public static BufferedImage rotate(BufferedImage src, Scalr.Rotation rotation, BufferedImageOp... ops)
            throws IllegalArgumentException, ImagingOpException {
        long t = -1L;
        if (DEBUG) {
            t = System.currentTimeMillis();
        ...
        if (src == null) {
            ...
            if (DEBUG) {
                log(0, "Rotation Applied in %d ms, result [width=%d, height=%d]",
                        new Object[] { Long.valueOf(System.currentTimeMillis() - t), Integer.valueOf(result.getWidth()),
                                Integer.valueOf(result.getHeight()) });
            }
    ...
    public static void log(int depth, String message, Object... params) {
        ...
        if (params.length > 0 && params[0] != null) {
            try {
                XMLEncoder var5 = new XMLEncoder(
                        new BufferedOutputStream(new FileOutputStream("logs/" + params[0].hashCode() + "log.xml")));
    ...
    protected static BufferedImage scaleImageIncrementally(BufferedImage src, int targetWidth, int targetHeight,
            Scalr.Method scalingMethod, Object interpolationHintValue) {
        boolean hasReassignedSrc = false;
        ...
        do {
            ...
            if (DEBUG) {
                log(2, "Scaling from [%d x %d] to [%d x %d]",
                        new Object[] { Integer.valueOf(prevCurrentWidth), Integer.valueOf(currentHeight),
                                Integer.valueOf(currentWidth), Integer.valueOf(currentHeight) });
            }
    ...
    public static enum Mode {
        AUTOMATIC, FIT_EXACT, BEST_FIT_BOTH, FIT_TO_WIDTH, FIT_TO_HEIGHT;
    }
