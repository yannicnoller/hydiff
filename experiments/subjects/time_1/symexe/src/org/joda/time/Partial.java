    ...
    public Partial(DateTimeFieldType[] types, int[] values, Chronology chronology) {
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            if (i > 0) {
                ...
                boolean tmpOld = compare < 0 || (compare != 0 && loopUnitField.isSupported() == false);
                boolean tmpNew = compare < 0;
                if (change(tmpOld, tmpNew)) { // changed condition
                    ...
                    if (types[i - 1].getRangeDurationType() == null) {
                        if (loopType.getRangeDurationType() == null) {
                            String exceptionMsg = "";
                            if (execute(OLD)) {
                                exceptionMsg = "Types array must not contain duplicate: " + loopType.getName();
                            }
                            if (execute(NEW)) {
                                exceptionMsg = "Types array must not contain duplicate: " +
                                        types[i - 1].getName() + " and " + loopType.getName();
                            }
                            throw new IllegalArgumentException(exceptionMsg); // changed message
                        }
                    } else {
                        if (loopType.getRangeDurationType() == null) {
                            throw new IllegalArgumentException("Types array must be in order largest-smallest: " +
                ...
                for (; i < iTypes.length; i++) {
                    ...
                    if (loopUnitField.isSupported()) {
                        ...
                        if (compare > 0) {
                            ...
                            if (change(false, fieldType.getRangeDurationType() == null)) { // changed added guarded break
                                break;
                            }
