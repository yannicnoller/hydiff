...
import test.org.apache.commons.math.special.Gamma1;
...
public class GammaDistributionImpl extends AbstractContinuousDistribution
    ...
    public double cumulativeProbability(double x) throws MathException{
        ...
        if (x <= 0.0) {
            ...
            ret = Gamma1.regularizedGammaP(getAlpha(), x / getBeta());
        }
    ...
    public double density(Double x) {
        if (x < 0) return 0;
        return Math.pow(x / getBeta(), getAlpha() - 1) / getBeta() * Math.exp(-x / getBeta()) / Math.exp(Gamma1.logGamma(getAlpha()));
    }
