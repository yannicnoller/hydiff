...
public class TCAS_V4 {
	...
	private boolean Non_Crossing_Biased_Climb(){
	    ...
	    if (upward_preferred){
	        ...
	        boolean tmp = change((Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= ALIM()), (Cur_Vertical_Sep >= MINSEP) || (Up_Separation >= ALIM()));
	    	result = Own_Above_Threat() && tmp; /* logic change  (Cur_Vertical_Sep >= MINSEP) (&& -> ||) (Up_Separation >= ALIM()) */
	    }
	    return result;
	}
